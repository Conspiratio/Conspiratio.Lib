...
- Aggressive KI (Sabotage + Anschwärzen): KI-Spieler mit schlechter Beziehung zum aktiven Menschen setzen jetzt selbst Saboteure gegen dessen Besitz ein oder schwärzen ihn bei anderen Würdenträgern an — verstärkt, wenn sie Beweise gegen ihn haben. Beide Mechaniken spiegeln bestehende (bisher nur Mensch→KI nutzbare) Mechanismen: Sabotage nutzt dieselbe Chance-/Schadensformel wie bisher (`ZugNachrichtenManager.ErmittleGegnerischeSabotageNachrichten`, neues Feld `HumSpieler._gegnerischeSabotagen`), Anschwärzen wurde dafür von seinem UI-Zustand entkoppelt (`DynamischeSpieldaten.AnschwaerzenAusfuehren`) und um eine Beweisgewichtung erweitert: Beweise senken die Glaubwürdigkeits-Schwelle des Adressaten von 80 auf minimal 50 (Quelle beim menschlichen Täter die bestehende Spionage, bei der KI ihre Ground-Truth-Deliktpunkte des Opfers) — davon profitiert auch der bestehende Mensch→KI-Pfad. Angeschwärzt wird nur bei KI-Amtsträgern. Neuer `AggressionManager` prüft dafür am selben Zugbeginn-Zeitpunkt wie die bestehende KI-Beleidigung (`PruefeKiBeleidigtSpieler`, dessen Feindseligkeits-Formel jetzt als `BerechneKiFeindseligkeitChance` wiederverwendbar ist) die drei feindseligsten KIs unabhängig voneinander; jede KI führt höchstens eine der drei feindlichen Aktionen pro Zug gegen denselben Menschen aus. Die Formel liefert eine Chance je *Kandidat* — sie für alle ~390 KIs einzeln auszuwürfeln ergäbe das Hundertfache an Ereignissen (gemessen 4–14 statt ~0,2 Aktionen pro Zug).
- Eine einzige Einstellung für die Aggressivität der KI-Spieler: `Spieleinstellungen.KiAggressivitaetProzent` (1–100 %, Standard 50) steuert jetzt sämtliche feindseligen und militärischen KI-Aktivitäten. Sie ersetzt das im Godot-Client nie gesetzte Feld `AggressivitaetKISpieler` (samt `EnumSchwierigkeitsgrad`, beide entfallen ersatzlos) und die zu eng gefasste `KiAktivitaetProzent`, die nur Räuberlager und Zollburgen betraf. Der eingefrorene Legacy-WinForms-Client setzt und liest `AggressivitaetKISpieler` dagegen weiterhin (`frmEinstellungen.cs:125,133,141`, `Main.cs:6067`) und kompiliert deshalb ab dieser Version nicht mehr – hingenommen, weil der WinForms-Client laut Projektkonvention nur noch Referenzimplementierung ist und keine Features mehr erhält. Die Einstellung wirkt an zwei Stellen: `KISpieler.GetBosheit` verschiebt den ausgewürfelten Charakterwert um `Aggressivität − 50` (auf 0–100 begrenzt) und erfasst damit alle davon abhängigen Mechaniken. Spürbar ist das bei der Gegnerstärke in Duellen (gewürfelt wie im Wortgefecht) sowie bei KI-Straftaten und Deliktpunkten. Wie oft eine KI dagegen beleidigt, sabotiert oder anschwärzt, hängt in der zugrunde liegenden Feindseligkeits-Formel (`BerechneKiFeindseligkeitChance`) ganz überwiegend von der Beziehung zum Spieler ab (Gewicht 13 gegenüber 1 für die Bosheit) – über den gesamten Reglerbereich verschiebt sich diese Häufigkeit dadurch nur um rund einen Prozentpunkt. `DynamischeSpieldaten.InterpoliereNachAggressivitaet` macht daneben die früheren Stufen Niedrig/Mittel/Hoch bei Gerichtsurteilen, Amtsenthebungen und Anklagen zu einer stufenlosen Skala. Bei 50 % verhält sich jede einzelne Stelle exakt wie bisher. Weil die Bosheit beim *Lesen* moduliert wird und nicht beim Auswürfeln, wirkt eine Änderung sofort und rückwirkend – auf bereits existierende KI-Spieler, im laufenden Spiel und in geladenen Spielständen; der gespeicherte Charakterwert bleibt unangetastet, die Streuung zwischen den KIs erhalten. Alte Spielstände (Feld fehlt, Wert 0) werden wie 50 % behandelt. Ausgenommen ist das Werbegeschenk der Brautwerbung (`FamilieManager.GibGeschenk`): Es ist keine Feindseligkeits-Mechanik und liest deshalb weiterhin den unmodulierten Rohwert über das neue `KISpieler.GetBosheitRoh()`.
...
- Fehlerbehebung Werkstatt-Verkaufspreis: `HandelsManager.GetWerkstattVerkaufspreis` gibt jetzt drei Viertel des **ungestaffelten** Grundpreises der Ware zurück statt drei Viertel des gestaffelten Kaufpreises. Der Staffelfaktor sitzt auf dem Grundpreis der jeweiligen Ware (Stufe 1 = 2 000, Stufe 2 = 10 000, Stufe 3 = 40 000), der Besitz-Zähler läuft aber reichsweit über alle Betriebe – der Erlös hing damit nicht daran, was ein Betrieb gekostet hatte, sondern daran, wie viele *andere* Betriebe man gerade besass. Daraus liess sich eine Geldpumpe bauen (billige Stufe-1-Betriebe verkaufen, einen teuren Stufe-3-Betrieb günstig kaufen, die billigen zurückkaufen, den teuren überteuert verkaufen). Jetzt verliert jeder Rückkauf-Zyklus, auch über Warenstufen hinweg.
- Fehlerbehebung Export: `BuchManager` bucht die exportierte Menge jetzt in den Vorrat der **Zielstadt** statt in den der Herkunftsstadt (bewusste Abweichung vom WinForms-Original). Bisher sättigte die Zielstadt nie, während das Exportvolumen den Preis in der eigenen Stadt drückte, in der gar nicht verkauft wurde – der Sättigungsabschlag griff auf dem Exportweg also überhaupt nicht. Der Umsatz bleibt bewusst bei der Herkunftsstadt, weil Verkaufssteuer und Kirchenzehnt am Kontor des Spielers erhoben werden.
- `DynamischeSpieldaten.ReichtumWachstumAktRundenEnde` würfelt jetzt je Stadt und Runde immer genau einmal und vergleicht danach. Zuvor entschied die Kurzschlussauswertung zustandsabhängig, ob überhaupt gewürfelt wurde, was den nachfolgenden Zufallsstrom verschob und Läufe mit festem Startwert unreproduzierbar machte.
- Fehlerbehebung Gesamtvermögen (Nebenbefund aus Review): `HumSpieler.GetGesamtVermoegen` zählte Werkstätten über eine Schleife, die beim internen Werkstattindex 1 statt 0 begann. Das Feld `_spielerHatInStadtXWerkstaettenY` ist aber 0-basiert (Platz 1 einer Stadt liegt auf Index 0) – Platz 1 jeder Stadt fehlte dadurch vollständig im Gesamtvermögen, und die zur Bewertung herangezogene Rohstoff-ID passte wegen des Versatzes ohnehin nicht zum geprüften Werkstattplatz. Behoben nach dem bereits vorhandenen, korrekten Muster aus `ErmittleLagerplatzInStadt` (0-basierter Werkstattindex, `GetSingleRohstoff(index + 1)` für den zugehörigen Rohstoff).
- Fehlerbehebung Gesamtvermögen, Teil 2 (weiterer Nebenbefund aus demselben Review): Direkt daneben las dieselbe Methode den Lagerbestand über `_hatInStadtXMengeYRohstoffe[i, j]` mit dem 0-basierten Werkstattindex `j` statt über die tatsächliche Rohstoff-ID – das Feld ist aber wie überall sonst im Code (`GetStadtRohstoffAnzahl`, `SetStadtRohstoffAnzahl`) über `[stadtID, rohID]` indiziert. Bei einer Rohstoff-ID oberhalb der Werkstattplatzzahl (0–5) wurde der Bestand dadurch am falschen, stets leeren Index abgelesen und fehlte komplett im Gesamtvermögen.
- Fehlerbehebung Gesamtvermögen, Teil 3 (weiterer Nebenbefund): `HumSpieler.GetGesamtVermoegen` bewertete eine besessene Werkstätte mit 70 % ihres Grundpreises, obwohl der Kommentar an der Stelle ausdrücklich den tatsächlichen Verkaufserlös meinte – der beträgt aber, wie `HandelsManager.GetWerkstattVerkaufspreis` seit der Geldpumpen-Korrektur oben zeigt, drei Viertel (75 %) des ungestaffelten Grundpreises. Das Gesamtvermögen lag dadurch je Werkstätte um 5 Prozentpunkte des Grundpreises zu niedrig (Stufe 1: 1 400 statt 1 500; Stufe 3: 28 000 statt 30 000) – mit Auswirkung auf Schuldenprozess, Bestenliste, Auftragsziele und Statistik. Neue Methode `Rohstoff.GetWSVerkaufspreis()` bündelt Kauf- und Verkaufswert derselben Ware an einer Stelle im Domänenmodell; `HandelsManager.GetWerkstattVerkaufspreis` und `GetGesamtVermoegen` nutzen sie jetzt beide, sodass Vermögensbewertung und echter Verkauf nicht mehr auseinanderlaufen können. `HumSpieler` ruft dafür weiterhin keinen Manager auf – die Regel sitzt bewusst auf dem Domänenobjekt, nicht in der Managerschicht darüber.
...
- Aggressive AI (sabotage + slander): AI players with a bad relationship to the active human now unleash saboteurs against their possessions or slander them to other officeholders — strengthened when they hold evidence against them. Both mechanics mirror existing (previously human→AI-only) mechanisms: sabotage reuses the same chance/damage formula as before (`ZugNachrichtenManager.ErmittleGegnerischeSabotageNachrichten`, new `HumSpieler._gegnerischeSabotagen` field), slander was decoupled from its UI state for this (`DynamischeSpieldaten.AnschwaerzenAusfuehren`) and gained evidence weighting: evidence lowers the target's credibility threshold from 80 down to a minimum of 50 (sourced from existing espionage for a human accuser, from the victim's ground-truth guilt score for an AI accuser) — the existing human→AI path benefits from this too. Slander is only ever addressed to AI officeholders. A new `AggressionManager` checks the three most hostile AIs independently of one another at the same turn-start point as the existing AI insult (`PruefeKiBeleidigtSpieler`, whose hostility formula is now reusable as `BerechneKiFeindseligkeitChance`); each AI performs at most one of the three hostile actions per turn against the same human. The formula yields a chance per *candidate* — rolling it for each of the ~390 AIs separately would produce a hundredfold event rate (measured 4–14 instead of ~0.2 actions per turn).
- A single setting for AI aggressiveness: `Spieleinstellungen.KiAggressivitaetProzent` (1–100 %, default 50) now governs all hostile and military AI activity. It replaces the field `AggressivitaetKISpieler`, never assigned by the Godot client (along with `EnumSchwierigkeitsgrad`, both dropped), and the too-narrow `KiAktivitaetProzent`, which only affected robber camps and toll castles. The frozen legacy WinForms client, however, still sets and reads `AggressivitaetKISpieler` (`frmEinstellungen.cs:125,133,141`, `Main.cs:6067`) and therefore no longer compiles against this version – accepted, since per project convention the WinForms client is reference-implementation-only and receives no further features. The setting takes effect in two places: `KISpieler.GetBosheit` shifts the AI's rolled character value by `aggressiveness − 50` (clamped to 0–100), thereby covering every mechanic that depends on it. This is noticeable for opponent strength in duels (rolled as well as in the verbal duel) and for AI crimes and guilt points. How often an AI insults, sabotages or slanders, by contrast, is governed almost entirely by its relationship to the player in the underlying hostility formula (`BerechneKiFeindseligkeitChance`, weight 13 versus 1 for malice) – across the setting's full range that frequency shifts by only about one percentage point. `DynamischeSpieldaten.InterpoliereNachAggressivitaet` separately turns the former Low/Medium/High tiers for court verdicts, removals from office and charges into a continuous scale. At 50 % every single one of these behaves exactly as before. Because malice is modulated when *read* rather than baked in when rolled, a change takes effect immediately and retroactively – on existing AI players, in a running game and in loaded savegames; the stored character value is left untouched and the spread between AIs preserved. Old savegames (field absent, value 0) are treated as 50 %. Exempted is the courtship gift mechanic (`FamilieManager.GibGeschenk`): it is not a hostility mechanic, so it keeps reading the unmodulated raw value via the new `KISpieler.GetBosheitRoh()`.
...
- Fix workshop sale price: `HandelsManager.GetWerkstattVerkaufspreis` now returns three quarters of the **unscaled** base price of the goods instead of three quarters of the scaled purchase price. The scaling factor applies to each type of goods (tier 1 = 2,000, tier 2 = 10,000, tier 3 = 40,000) while the ownership counter spans the whole realm, so the proceeds depended on how many *other* workshops were owned rather than on what the workshop had cost. That allowed an unbounded money pump (sell cheap tier-1 workshops, buy an expensive tier-3 one cheaply, buy the cheap ones back, sell the expensive one at an inflated price). Every buy-back cycle now loses money, across tiers as well.
- Fix export: `BuchManager` now books the exported quantity into the stock of the **destination** city instead of the city of origin (a deliberate deviation from the WinForms original). Previously the destination never saturated while the export volume pushed down the price in the player's own city, where nothing had been sold – so the saturation discount never applied to exports at all. Turnover deliberately stays with the city of origin, because sales tax and church tithe are levied at the player's own branch.
- `DynamischeSpieldaten.ReichtumWachstumAktRundenEnde` now always rolls exactly once per city and round and compares afterwards. Short-circuit evaluation previously made the roll itself depend on game state, which shifted the subsequent random stream and broke the reproducibility of fixed-seed runs.
- Fix total wealth (side finding from a review): `HumSpieler.GetGesamtVermoegen` counted workshops via a loop that started at internal workshop index 1 instead of 0. The `_spielerHatInStadtXWerkstaettenY` field is 0-based (a city's slot 1 sits at index 0), so slot 1 of every city was missing entirely from the total wealth, and due to the offset the resource ID used for valuation didn't even match the workshop slot being checked in the first place. Fixed following the already-correct pattern in `ErmittleLagerplatzInStadt` (0-based workshop index, `GetSingleRohstoff(index + 1)` for the matching resource).
- Fix total wealth, part 2 (another side finding from the same review): right next to it, the same method read stock via `_hatInStadtXMengeYRohstoffe[i, j]` using the 0-based workshop index `j` instead of the actual resource ID – but that field is indexed by `[cityID, resourceID]` everywhere else in the code (`GetStadtRohstoffAnzahl`, `SetStadtRohstoffAnzahl`). For a resource ID above the workshop slot count (0–5), the stock was read from the wrong, always-empty index and was missing entirely from the total wealth.
- Fix total wealth, part 3 (another side finding): `HumSpieler.GetGesamtVermoegen` valued an owned workshop at 70 % of its base price, even though the comment right there explicitly meant the actual sale proceeds – which, as `HandelsManager.GetWerkstattVerkaufspreis` shows after the money-pump fix above, are three quarters (75 %) of the unscaled base price. Total wealth was therefore too low by 5 percentage points of the base price per workshop (tier 1: 1,400 instead of 1,500; tier 3: 28,000 instead of 30,000) – affecting the debt-tower process, the leaderboard, mission goals and statistics. New method `Rohstoff.GetWSVerkaufspreis()` gathers the purchase and sale value of the same goods in one place in the domain model; both `HandelsManager.GetWerkstattVerkaufspreis` and `GetGesamtVermoegen` now use it, so the wealth valuation and the actual sale can no longer drift apart. `HumSpieler` still calls no manager for this – the rule deliberately lives on the domain object, not in the manager layer above it.
...
- Reproduzierbare Spiele: `SW.Statisch.SetRnd(int seed)` setzt den Zufallsgenerator auf einen festen Startwert. Bisher war `Rnd` nur lesbar (privater Setter), sodass sich ein Spielverlauf nicht wiederholen ließ. Gedacht für automatisierte Durchläufe und Tests, in denen ein Fehlschlag sonst nicht nachstellbar ist; der Aufruf muss nach `Initialisieren` erfolgen, weil dieses den Generator neu anlegt.
- Katastrophen (WinForms-Issue #37, Vorbild „Die Fugger 2"): Neuer `KatastrophenManager`. Sturm, Flut, Brand, Erdbeben und Pest suchen die Städte heim – selten (rund alle vier bis fünf Jahre), dann aber heftig. Welche Stadt welche Art treffen kann, steuern die seit jeher in den Spieldaten hinterlegten Anfälligkeiten je Stadt (`Stadt.GetKatastrophen`, 0–100 je Art): Eine Stadt mit 0 wird davon nie heimgesucht. Eine Katastrophe trifft eine einzelne Stadt, alle anfälligen Städte einer Grafschaft (35 %) oder des ganzen Reichs (15 %). Betroffene Städte verlieren 8–20 % ihrer Einwohner (Pest doppelt), 1–2 Punkte Reichtum und 20–40 % ihrer Warenvorräte; die Knappheit hebt die Preise um 2–4 je Ware. Auch die Spieler trifft es: Eingelagerte Waren gehen im selben Umfang verloren, Wohnsitze in der Stadt verlieren 10–25 Prozentpunkte Zustand, und wer bei einer Pest dort einen Wohnsitz hat, büßt Gesundheit ein.
- Fehlerbehebung Privilegien: Das Privileg „Untergebene" blieb nach einem Amtswechsel sichtbar, wenn das neue Amt keine Untergebenen hat. In `PrivilegienAktualisieren` deckte die Zuweisung nur den Fall „Untergebene vorhanden" ab, sodass ein einmal gesetztes `true` aus einem früheren Amt stehen blieb – der Spieler konnte die Untergebenen-Ansicht dann in einem Amt öffnen, das gar keine hat. Die Zuweisung erfasst jetzt beide Fälle.
- Erpressen (WinForms-Issue #13, Konzept von PommBaer): Neuer `ErpressungManager`. Wer über seine Spione genug Belastendes gegen einen Amtsträger gesammelt hat, kann ihn erpressen und einige Jahre lang dessen Amtsprivilegien mitnutzen. Die Beweispunkte sind die bereits vorhandene, aufsummierte Beweismächtigkeit der Spionage; nötig sind 3 (Stadt), 5 (Grafschaft) bzw. 7 (Königreich) Punkte. Die Erfolgsaussicht beträgt 75 % plus 4 % je überzähligem Beweispunkt, minus 15 % je Titelstufe, die das Ziel über dem Erpresser steht (plus 3 % je Stufe darunter), minus 5 % bei gleicher Religion. Eine gelungene Erpressung wirkt 4/3/2 Jahre plus 0–3/0–2/0–1 Jahre Zufall und verbraucht die Beweise; eine misslungene lässt sie verfallen und kostet 10 Beziehungspunkte. Mehrere Erpressungen laufen parallel und enden zum Jahreswechsel (`RundenManager.BeendeAbgelaufeneErpressungen`). Menschliche Opfer entscheiden selbst (neue Schnittstelle `IErpressungDialog`): Wer ablehnt, lässt dem Erpresser die Beweise – sie taugen dann für eine Anklage. Der Erpresste verliert seine aktiv nutzbaren Amtsprivilegien (Gesetze, Urteile, Stiftungen, Anschläge), behält aber die passiven Vorteile. Erpressung ist selbst strafbar: neues Strafgesetz (Index 25), das Gericht und Jurist ohne weitere Anpassung erfassen – die Gesetzestabelle ist mit 100 Plätzen fest dimensioniert, alte Spielstände bleiben unberührt. Neu in `DynamischeSpieldaten`: `WirdErpresst`, `GetErpresserVon`, `GetAmtsPrivilegien`; erpresste Amtsträger tragen in den Ämterlisten einen Dolch hinter dem Namen. Der alte, wirkungslose `Erpressen`-Rumpf entfällt.
- Interaktives Duell (Issue #17, Vorbild „Monkey Island 1"): Neuer `WortgefechtManager` – ein Duell wird nicht mehr gewürfelt, sondern als Schlagabtausch von Beleidigungen gespielt. Ein Wortwechsel besteht aus zwei Schritten: Der Angreifer wählt aus mindestens drei Beleidigungen (`NaechsterAngriff`/`WaehleAngriff`), der Angegriffene muss unter mindestens drei Kontern den passenden erkennen (`WerteKonterAus`); die falschen Antworten sind die Konter anderer Beleidigungen. Wer zuerst drei Wortwechsel gewinnt, gewinnt das Duell; kein Spruchpaar wiederholt sich. Gegen einen menschlichen Gegner wählen beide Seiten selbst (Hot-Seat), gegen die KI entscheidet beim Kontern ein Würfel, gewichtet mit der Fechtfähigkeit. Diese wirkt hybrid: gedeckelter Treffervorsprung plus höhere Trefferquote der eigenen Beleidigungen. Gemessen gegen einen mittleren Gegner gewinnt perfektes Kontern ohne Fechtunterricht rund 70 % der Duelle und mit Fechtfähigkeit 150 praktisch alle, während durchweg falsche Konter selbst mit maximalem Unterricht nur rund 40 % gewinnen. `FechtDuellManager` wurde dafür aufgetrennt: `BerechneSiegchance` und `WendeDuellAusgangAn` sind öffentlich, `FuehreDuellDurch` (gewürfeltes Duell) verhält sich unverändert. `IDuellDialog` bietet nun `SpieleWortgefecht` (trägt das Duell aus) und `ZeigeAusgang` (zeigt die Folgen).
...
- Duelle erweitert (Issue #17): Eine Beleidigung ist jetzt ein Ehrenhandel mit Entscheidung. Wer einen Amtsträger beleidigt, sinkt deutlich in dessen Ansehen (Beziehungsverlust). Der Beleidigte entscheidet, ob er Satisfaktion verlangt – ein Duell im Morgengrauen (KI abhängig von ihrer Bosheit, menschliche Ziele per Dialog). Verzichtet er, leidet sein eigenes Ansehen bei den Amtsträgern seiner Amtsstufe (Stadt/Land/Reich). Selten beleidigt auch eine KI den Spieler (`PruefeKiBeleidigtSpieler`); die Chance hängt vor allem an ihrer Beziehung zum Spieler (niedriger Wert → höher, erst unter „neutral"), Bosheit spielt eine kleine Rolle – im Schnitt seltener als alle 15 Züge. Auch hier wählt der Spieler Duell oder Verzicht (Verzicht kostet Ansehen). Duelle zwischen zwei menschlichen Spielern vergleichen nun beide Fechtfähigkeiten.
- Duelle & Fechtunterricht (Issue #17, Vorbild „Die Fugger 2"): Neuer `FechtDuellManager`. Über Fechtunterricht steigert der Spieler seine `Fechtfaehigkeit` (neues Feld an `HumSpieler`) – der Preis pro Stunde steigt mit jeder genommenen Stunde. Ein Amtsträger kann wegen einer Beleidigung zum Duell gefordert werden (max. eines pro Zug); der Sieger ergibt sich aus der Fechtfähigkeit gegen eine aus der KI-Bosheit abgeleitete Gegnerstärke plus Zufall. Der Verlierer verliert Gesundheit; sinkt sie unter 30, verliert er sein Amt (Neuwahl über `AmtVonXfreigeben`) – symmetrisch, auch der herausfordernde Spieler kann verlieren. Zwei synthetische Privilegien-Einträge (`FechtunterrichtPrivilegId` 10002, `DuellPrivilegId` 10003) sowie der Personen-Karten-Modus 14 (`KontrahentenManager`/`AemterEbeneManager`). Alte Spielstände starten mit Fechtfähigkeit 0.
- Fehlerbehebung Spieltipps: `Tipps[16]` war doppelt belegt – der Kredit-Hinweis überschrieb den Tipp „Verkaufserlös nimmt bei wiederholtem Verkauf ab", sodass dieser nie erschien. Der Kredit-Hinweis liegt jetzt auf einem eigenen Index (30), `TippsMaxIndex` entsprechend auf 30 erhöht; beide Tipps sind wieder vorhanden.
- Robustheit Untergebene: `DynamischeSpieldaten.GetUntergebene` legte die Untergebenen-Ämter und -Inhaber in zwei fest 7 Einträge große Arrays ab, während `ufinalID` je untergeordnetem Amt zusätzlich über alle Städte/Länder eines Gebiets multipliziert. In der aktuellen Ämtertabelle passt das gerade so (maximal 6 Einträge), der Puffer ist aber nur einen Slot groß – schon ein zusätzliches Unteramt oder eine 5. Stadt je Land würde zu einer `IndexOutOfRangeException` beim Öffnen des Privilegs „Untergebene" führen. Die Arrays werden nun sicher nach den Max-Konstanten dimensioniert (analog zum Wahlen-Fix). Gefunden bei einem gezielten Scan nach fest verdrahteten Array-Größen.
- Privileg „Vergifteter Wein" (Kellermeister, Amt 23) vollständig verdrahtet: `PrivVergifteterWein.PrivExecute` öffnet nun die Personen-Karte im Zielauswahl-Modus 12 (sofern noch keine Vergiftung vorbereitet ist), und `KontrahentenManager.PersonWasMachen` führt für Modus 12 `WeinVergiften` aus (5.000 Taler, Ziel für dieses Jahr vormerken). Die Auflösung zum Zugende (`FuehreVergiftetenWeinDurch`) und die Erfolgschance waren bereits vorhanden. `AemterEbeneManager.GetTitel` liefert die passende Kopfzeile „Wein vergiften in …". Zuvor zeigte das Privileg nur einen Text ohne Wirkung.
- Aufräumen: Die ungenutzte Methode `ZugNachrichtenManager.FuehreTodDesAktivenSpielersDurch` entfernt (toter Code samt veraltetem TODO – Tod und Erbfolge des Spielers laufen längst über `FamilieManager.FuehreTestamentAus`).
...
**[EN]**
- Fix, offices: `AemterManager.FuelleRestlicheAemter` blindly rolled one of the AI slots as the winner of every registered election. But `IstDieWahlVoll` only states that the election exists at all – not that anyone applied. When the roll hit an empty slot (candidate 0), `VergebeAmt` threw a NullReferenceException that aborted the entire turn of the year, leaving the game stuck. The winner is now drawn only from the candidates actually present; with none, the office stays vacant and the election comes up again next year. Found by the Godot client's automated play-through.
- Reproducible games: `SW.Statisch.SetRnd(int seed)` seeds the random generator. Until now `Rnd` was read-only (private setter), so a run could not be repeated. Intended for automated play-throughs and tests where a failure would otherwise not be reproducible; call it after `Initialisieren`, which creates a fresh generator.
- Catastrophes (WinForms issue #37, inspired by "Die Fugger 2"): new `KatastrophenManager`. Storm, flood, fire, earthquake and plague strike the cities – rarely (roughly every four to five years) but hard. Which city can be hit by which kind is driven by the per-city susceptibilities that have always been in the game data (`Stadt.GetKatastrophen`, 0–100 per kind): a city with 0 is never struck by that kind. A catastrophe hits a single city, all susceptible cities of a county (35 %) or of the whole realm (15 %). Affected cities lose 8–20 % of their inhabitants (plague twice as much), 1–2 points of wealth and 20–40 % of their goods; the resulting scarcity raises prices by 2–4 per good. Players are hit too: stored goods are lost to the same extent, residences in the city lose 10–25 percentage points of condition, and anyone holding a residence there during a plague loses health.
- Fix, privileges: the "Untergebene" privilege stayed visible after changing office when the new office has no subordinates. In `PrivilegienAktualisieren` the assignment only covered the "has subordinates" case, so a `true` set for an earlier office remained – the player could then open the subordinates view in an office that has none. The assignment now covers both cases.
- Blackmail (WinForms issue #13, concept by PommBaer): new `ErpressungManager`. Anyone who has gathered enough incriminating material against an office-holder through their spies can blackmail them and use that office's privileges for a few years. The evidence points are the already existing accumulated evidence strength of the espionage; 3 (city), 5 (county) or 7 (kingdom) points are required. The chance of success is 75 % plus 4 % per surplus evidence point, minus 15 % per title rank the target stands above the blackmailer (plus 3 % per rank below), minus 5 % for the same religion. A successful blackmail lasts 4/3/2 years plus 0–3/0–2/0–1 random years and consumes the evidence; a failed one destroys it and costs 10 relationship points. Several blackmails run in parallel and end at the turn of the year (`RundenManager.BeendeAbgelaufeneErpressungen`). Human victims decide for themselves (new `IErpressungDialog` interface): whoever refuses leaves the evidence with the blackmailer – it can then be used for a prosecution. The blackmailed party loses the actively usable privileges of their office (laws, verdicts, endowments, attacks) but keeps the passive benefits. Blackmail is itself a crime: a new penal law (index 25) that court and jurist pick up without further changes – the law table has a fixed capacity of 100 entries, so old savegames are unaffected. New in `DynamischeSpieldaten`: `WirdErpresst`, `GetErpresserVon`, `GetAmtsPrivilegien`; blackmailed office-holders carry a dagger after their name in the office lists. The old, ineffective `Erpressen` stub has been removed.
- Interactive duel (issue #17, inspired by "Monkey Island 1"): new `WortgefechtManager` – a duel is no longer rolled but played out as an exchange of insults. Each exchange has two steps: the attacker picks from at least three insults (`NaechsterAngriff`/`WaehleAngriff`), the defender must spot the fitting one among at least three comebacks (`WerteKonterAus`); the wrong answers are the comebacks of other insults. Whoever wins three exchanges first wins the duel; no pair repeats. Against a human opponent both sides choose for themselves (hot-seat); against the AI a die weighted by fencing skill decides its comeback. Fencing skill acts as a hybrid: a capped head start plus a higher hit rate for one's own insults. Measured against a mid-level opponent, perfect countering wins about 70 % of duels with no fencing lessons at all and practically all of them at skill 150, while consistently wrong comebacks win only about 40 % even with maximum training. `FechtDuellManager` was split accordingly: `BerechneSiegchance` and `WendeDuellAusgangAn` are public, and `FuehreDuellDurch` (rolled duel) behaves unchanged. `IDuellDialog` now offers `SpieleWortgefecht` (fights the duel) and `ZeigeAusgang` (shows the consequences).
...
- Duels extended (issue #17): an insult is now an affair of honour with a decision. Whoever insults an office-holder drops significantly in their regard (relationship loss). The insulted party decides whether to demand satisfaction – a duel at dawn (AI depending on its malice, human targets via dialog). If they decline, their own reputation suffers among the office-holders of their office tier (town/land/realm). Rarely an AI also insults the player (`PruefeKiBeleidigtSpieler`); the chance mainly depends on its relationship to the player (lower → higher, rising only below "neutral"), malice plays a small role – on average rarer than once every 15 turns. Here too the player chooses duel or decline (declining costs reputation). Duels between two human players now compare both fencing skills.
- Duels & fencing lessons (issue #17, inspired by "Die Fugger 2"): new `FechtDuellManager`. Via fencing lessons the player raises their `Fechtfaehigkeit` (new field on `HumSpieler`) – the price per lesson rises with each lesson taken. An office-holder can be challenged to a duel over an insult (at most one per turn); the winner follows from the fencing skill against an opponent strength derived from the AI's malice plus randomness. The loser loses health; if it drops below 30 they lose their office (re-election via `AmtVonXfreigeben`) – symmetric, the challenging player can lose too. Two synthetic privilege entries (`FechtunterrichtPrivilegId` 10002, `DuellPrivilegId` 10003) plus person-map mode 14 (`KontrahentenManager`/`AemterEbeneManager`). Old savegames start with fencing skill 0.
- Bugfix game tips: `Tipps[16]` was assigned twice – the loan hint overwrote the tip "sales revenue drops with repeated selling", so the latter never appeared. The loan hint now has its own index (30), `TippsMaxIndex` raised to 30 accordingly; both tips are present again.
- Robustness subordinates: `DynamischeSpieldaten.GetUntergebene` stored the subordinate offices and holders in two fixed 7-entry arrays, while `ufinalID` additionally multiplies each subordinate office across all towns/lands of a region. In the current office table this just fits (at most 6 entries), but the headroom is only one slot – a single additional subordinate office or a 5th town per land would cause an `IndexOutOfRangeException` when opening the "subordinates" privilege. The arrays are now sized safely according to the max constants (analogous to the elections fix). Found during a targeted scan for hard-coded array sizes.
- Privilege "poisoned wine" (cellar master, office 23) fully wired up: `PrivVergifteterWein.PrivExecute` now opens the person map in target-selection mode 12 (unless a poisoning is already prepared), and `KontrahentenManager.PersonWasMachen` runs `WeinVergiften` for mode 12 (5,000 taler, marks the target for this year). The turn-end resolution (`FuehreVergiftetenWeinDurch`) and success chance already existed. `AemterEbeneManager.GetTitel` returns the matching header "Wein vergiften in …". Previously the privilege only showed a text with no effect.
- Cleanup: removed the unused method `ZugNachrichtenManager.FuehreTodDesAktivenSpielersDurch` (dead code with a stale TODO – the player's death and inheritance have long gone through `FamilieManager.FuehreTestamentAus`).
